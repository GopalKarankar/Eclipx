...
const Comment = ({comment, setOpenCommentId, openCommentId, videoId}) => {
    ...
    useEffect(() => {
      ...
      const fetchComment = async () => {
        ...
        try {
            const res = await axios.get(`${process.env.BACKEND_URI}/api/users/find/${comment.userId}`);
    ...
    const deleteVideo = async () => {
      ...
      try {
        ...
        if(confirmDelete){
            ...
            const res = await axios.delete(`${process.env.BACKEND_URI}/api/comments/${comment._id}`);
