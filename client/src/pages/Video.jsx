...
const Video = () => {
  ...
  const [isViewCounted, setIsViewCounted] = useState(false);
        ...
        const incrViewCount = async () => {
            ...
            await axios.put(`process.env.BACKEND_URI/api/videos/view/${currentVideo?._id}`,
              {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () =>{
      ...
      try{
        ...
        const videoRes = await axios.get(`process.env.BACKEND_URI/api/videos/find/${path}`);
        const channelRes = await axios.get(`process.env.BACKEND_URI/api/users/find/${videoRes.data.userId}`);
  ...
  const handleLike = async () => {
    try {
      await axios.put(
        `process.env.BACKEND_URI/api/users/like/${currentVideo?._id}`,
        {},
        { withCredentials: true } 
      );
  ...
  const handleDislike = async () => {
    ...
    try {
      ...
      await axios.put(
        `process.env.BACKEND_URI/api/users/dislike/${currentVideo._id}`,
        {},
        { withCredentials: true } 
      );
  ...
  const handleSub = async () =>{
    ...
    try {
            ...
            if (currentUser.subscribedUsers.includes(channel._id)){
                ...
                await axios.put(
                  `process.env.BACKEND_URI/api/users/unsub/${channel._id}`,
                    {},
                    { withCredentials: true } 
                  );
                  ...
                  await axios.put(
                  `process.env.BACKEND_URI/api/users/sub/${channel._id}`,
                    {},
                    { withCredentials: true } 
                  );
  ...
  const handleSave = async () => {
      ...
      try {
        ...
        if (currentUser.savedVideos.includes(currentVideo._id)) {
            ...
            const res = await axios.put(`process.env.BACKEND_URI/api/users/unsavevideo/${currentVideo._id}`);
            ...
            const res = await axios.put(`process.env.BACKEND_URI/api/users/savevideo/${currentVideo._id}`);
